...
5. **Thiết kế (Architecture) - `techlead`**
   ...
   - **Review (HUMAN):** Yêu cầu Người dùng review bản thiết kế kỹ thuật.
   - **Vòng lặp (Loop):** Nếu HUMAN Reject, Techlead phải điều chỉnh lại kiến trúc. Chỉ khi HUMAN **Approve** mới chuyển sang bước tiếp theo.

6. **Vòng lặp (Loop): Backend**
   - **Git Branch (bắt buộc)**: Trước khi code, phải tạo nhánh mới dành riêng cho backend, ví dụ: `feature/<ten-feature>-backend`.
   - **Backend (code) - `backend-developer`:** Triển khai code luồng Backend, API, DB dựa trên bản thiết kế, luôn commit/push trên nhánh vừa tạo (không làm trực tiếp trên `main`).
   ...
   - _(Lặp lại cho đến khi Backend hoàn chỉnh)_
   - **Approval (HUMAN)**: HUMAN duyệt Task. **KHÔNG tự động merge**. Chờ lệnh hoặc HUMAN tự merge.

7. **Vòng lặp (Loop): Frontend**
   - **Git Branch (bắt buộc)**: Trước khi code, phải tạo nhánh mới dành riêng cho frontend, ví dụ: `feature/<ten-feature>-frontend`.
   - **Frontend (code) - `frontend-developer`:** Triển khai giao diện, kết nối API, luôn commit/push trên nhánh vừa tạo (không làm trực tiếp trên `main`).
   - **Config & Styling**: Cấu hình `.env` (VITE\_) và Tailwind v4 (nếu dùng) theo chuẩn mới.
   ...
   - _(Lặp lại cho đến khi Frontend hoàn chỉnh)_
