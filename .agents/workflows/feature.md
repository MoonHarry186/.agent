...
- Feature spec (BA): `docs/features/<feature-name>.md` (VN/EN)
- Task breakdown (PM): `docs/management/<feature-name>-tasks.md` (VN/EN)
...
8. **Task Tracking Update cho từng task (Required) - `project-manager`**

- Cập nhật trạng thái **task đang làm** trong `docs/management/<feature-name>-tasks.md`:
  - Task đang làm → `done` (hoặc đánh dấu `[x] Done` theo format của file tasks)
  - Nếu task bị blocked → ghi rõ reason + dependency
- Gắn link tham chiếu (tối thiểu):
  - Branch name / PR (nếu có)
  - QA checklist: `docs/qa/<feature-name>-qa.md`
  - Architecture/contract: `docs/architecture/<feature-name>.md`
- **Exit criteria**: file tasks đã được update trạng thái + có link QA/branch để trace.
